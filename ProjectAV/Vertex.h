...
#define LAYOUT_ELEMENT_TYPES \
	...
	X( Float4Color ) \
	X( BGRAColor ) \
	X( BoneIDs )      /* ADDED FOR SKinning */ \
	X( BoneWeights )  /* ADDED FOR SKinning */ \
	X( Count )

namespace Dvtx
{
	// Helper struct for passing bone data for a single vertex
	struct VertexBoneData {
		...
		void Add(UINT boneId, float weight) {
			if (weight <= 0.0f) return; // Ignore zero or negative weights
			...
			for (int i = 0; i < 4; ++i) {
				if (weights[i] == 0.0f) {
					ids[i] = boneId;
					weights[i] = weight;
					boneCount = (i + 1 > boneCount) ? i + 1 : boneCount;
					return;
				}
			}

			// If all slots are full, replace the one with the smallest weight if current weight is larger
			// (More sophisticated strategies exist, e.g. keeping the 4 largest)
			int smallestIdx = 0;
			for (int i = 1; i < 4; ++i) {
...
#undef X
		...
		template<> struct Map<BoneIDs> // ADDED
		{
			using SysType = DirectX::XMUINT4; // Using XMUINT4 for 4 uints
			static constexpr DXGI_FORMAT dxgiFormat = DXGI_FORMAT_R32G32B32A32_UINT; // Or R16G16B16A16_UINT if max bones < 65536, or R8G8B8A8_UINT if < 256
			static constexpr const char* semantic = "BLENDINDICES";
			static constexpr const char* code = "BI4";
			// No DVTX_ELEMENT_AI_EXTRACTOR, custom handling
		};
		template<> struct Map<BoneWeights> // ADDED
		{
			using SysType = DirectX::XMFLOAT4;
			static constexpr DXGI_FORMAT dxgiFormat = DXGI_FORMAT_R32G32B32A32_FLOAT;
		...
		template<> struct Map<Count>
		{
			...
			DVTX_ELEMENT_AI_EXTRACTOR( mFaces ) // This was likely an error or placeholder
		};
...
#undef X
	...
	public:
		...
		// Constructor for Assimp mesh, now takes pre-processed bone data
		VertexBuffer(VertexLayout layout_in, const aiMesh& mesh, const std::vector<VertexBoneData>& vertexBoneDataSource = {});
