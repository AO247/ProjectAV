...
namespace Bind
{
        ...
        : m_slot(slot)
    {
        ...
        cbd.MiscFlags = 0u;
        cbd.ByteWidth = sizeof(Transforms); // Size of our struct with the array
    ...
    {
        INFOMAN(gfx);
        Transforms transformsGpuData = {}; // Zero-initialize (or default if Transforms has constructor)

        size_t countToCopy = std::min(finalBoneMatricesData.size(), (size_t)MAX_SHADER_BONES_CB);
        for (size_t i = 0; i < countToCopy; ++i) {
            transformsGpuData.boneMatrices[i] = finalBoneMatricesData[i]; // Data is already transposed from AnimationComponent
        }
        // Fill remaining with identity if fewer bones than max (good practice)
    ...
    std::string BoneTransformCbuf::GenerateUID(UINT slot)
    {
        ...
        return typeid(BoneTransformCbuf).name() + "#s"s + std::to_string(slot); // Ensure unique UIDs if slot varies
    }
}